function FV_loadAnalyzedROI(handles)
...
if exist(roiFilename, 'file')
    ...
    disp(['File not found: ', roiFilename]);
    return;
end
...
if ~isempty(Aout.allROIInfo.ROI)
    UserData = rmfield(Aout.allROIInfo.ROI, 'includedZ');
...
if currentStruct.state.loadSettingOpt.value % if also loading settings. This is like loading default but only for selected fields.
