function fittedData = FV_fitBySingleExp(t,intensity, handles)
...
if beta0(2) <= 0
    ...
    state.fixTau1Opt.value = 0;
    state.fixTau2Opt.value = 0;
end
beta0(3) = 0;
beta0(4) = 0;
if beta0(6) <= 0
...
% But the old code in fact is right: lower value is higher weight in ss_nlinfit. So 1 is the lowest weight.
...
[betahat,R,J,converge] = ss_nlinfit(x, intensity, weight, @FV_expGauss, beta0);
...
fittedData.fittedYdata = FV_expGauss(betahat, x2);
...
function y = FV_expGauss(beta0, x)
%beta0(1): peak
%beta0(2): exp tau
%beta0(5): center
...
% % testing: paper has no square for y1...
% y1 = beta0(1)*exp(tau_g^2/2/tau1 - (x-deltapeak)/tau1);
% y2 = erfc((tau_g^2-tau1*(x-deltapeak))/(sqrt(2)*tau1*tau_g));
% y=y1.*y2;
% 
% %"Pre" pulse
% y1 = beta0(1)*exp(tau_g^2/2/tau1 - (x-deltapeak+pulseI)/tau1);
% y2 = erfc((tau_g^2-tau1*(x-deltapeak+pulseI))/(sqrt(2)*tau1*tau_g));
